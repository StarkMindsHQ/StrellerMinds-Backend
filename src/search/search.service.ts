...
export class SearchService implements OnModuleInit {
  ...
  async facetedSearch(
    facetDto: FacetedSearchDto,
    userId?: string,
  ): Promise<SearchResult & { facets: FacetResult[] }> {
    try {
      const cacheKey = this.generateCacheKey('facetedSearch', facetDto, userId);
      const cachedResult = await this.cacheService.get<SearchResult & { facets: FacetResult[] }>(
        cacheKey,
      );
      if (cachedResult) {
        return cachedResult;
  ...
  async getSuggestions(
    suggestionDto: SearchSuggestionDto,
  ): Promise<{ suggestions: SearchSuggestion[] }> {
    try {
      const cacheKey = this.generateCacheKey('suggestions', suggestionDto);
      const cachedResult = await this.cacheService.get<{ suggestions: SearchSuggestion[] }>(
        cacheKey,
      );
      if (cachedResult) {
        return cachedResult;
  ...
  async getRecommendations(
    ...
    try {
      ...
      const result = await this.searchRecommendationService.getPersonalizedRecommendations(
        recommendationDto,
        userId,
      );
  ...
  async getSearchAnalytics(params: {
    ...
    try {
      ...
      const recentSearches = await queryBuilder
        .orderBy('analytics.searchedAt', 'DESC')
        .limit(limit)
        .getMany();
  ...
  async indexCourse(course: Course): Promise<any> {
    try {
      ...
      const embedding = await this.searchMLService.generateEmbedding(
  ...
  async updatePopularSearches() {
    try {
      const popularQueries = await this.searchAnalyticsRepository
        ...
        .select('analytics.query', 'query')
        .addSelect('COUNT(*)', 'count')
        .where('analytics.searchedAt >= :date', {
  ...
  async getPopularSearchTerms(limit = 20): Promise<{ query: string; searchCount: number }[]> {
    ...
    const cachedTerms =
      await this.cacheService.get<{ query: string; searchCount: number }[]>(cacheKey);
    if (cachedTerms) {
      return cachedTerms;
    }
