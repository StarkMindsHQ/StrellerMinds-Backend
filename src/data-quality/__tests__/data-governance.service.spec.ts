...
describe('DataGovernanceService', () => {
  ...
  describe('getPolicies', () => {
    it('should return policies with filters', async () => {
      ...
      expect(mockQueryBuilder.where).toHaveBeenCalledWith('policy.category = :category', {
        category: 'retention',
      });
      expect(mockQueryBuilder.andWhere).toHaveBeenCalledWith('policy.status = :status', {
        status: 'active',
      });
    });
  ...
  describe('archiveOldPolicies', () => {
    it('should archive old policies', async () => {
      ...
      expect(mockQueryBuilder.where).toHaveBeenCalledWith('policy.updatedAt < :cutoffDate', {
        cutoffDate,
      });
      expect(mockQueryBuilder.andWhere).toHaveBeenCalledWith('policy.status != :status', {
        status: 'archived',
