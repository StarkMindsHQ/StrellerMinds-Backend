...
export class DataQualityReportingService {
  ...
  private async getIssuesByCategory(
    startDate: Date,
    endDate: Date,
  ): Promise<Record<string, number>> {
  ...
  private generateRecommendations(
    ...
    if (criticalIssues > 0) {
      recommendations.immediate.push(
        `Address ${criticalIssues} critical data quality issues immediately`,
      );
    }

    if (overallScore < 70) {
      recommendations.immediate.push(
        'Overall data quality score is below acceptable threshold (70%)',
      );
    }
    ...
    if (underPerformingCategories.length > 0) {
      recommendations.shortTerm.push(
        `Focus on improving ${underPerformingCategories.join(', ')} categories`,
      );
    }
