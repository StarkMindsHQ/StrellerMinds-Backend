...
export class RealTimeAnalyticsService {
  ...
  async getActiveUsers(): Promise<number> {
    const activeUserMetrics = Array.from(this.metricsBuffer.values()).filter(
      (metric) => metric.name === 'active_users',
    );

    return new Set(activeUserMetrics.map((metric) => metric.dimensions?.userId).filter(Boolean))
      .size;
  }
