    ...
    try {
      ...
      for (const metricName of query.metrics) {
        ...
        for (const metric of metrics) {
          const existingDataPoint = results.data.find(
            (dp) => dp.timestamp.getTime() === metric.timestamp.getTime(),
          );
        ...
        results.summary.aggregations[`${metricName}_avg`] =
          metrics.length > 0 ? totalValue / metrics.length : 0;
      }
