...
export class SkillAssessmentService {
  ...
  async submitAssessment(submitDto: SubmitAssessmentDto): Promise<AssessmentAttempt> {
    ...
    // Calculate score
    const { score, percentage, feedback } = await this.calculateScore(
      attempt.assessment,
      submitDto.answers,
    );
  ...
  private generateRecommendations(assessment: SkillAssessment, percentage: number): string[] {
    ...
    if (percentage < 50) {
      recommendations.push(
        'Consider reviewing the fundamental concepts before retaking the assessment.',
      );
      ...
      recommendations.push(
        'Great job! Consider taking more advanced assessments in this skill area.',
      );
