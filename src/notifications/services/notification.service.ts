...
export class NotificationService {
  ...
  async sendNotification(dto: CreateNotificationDto): Promise<Notification> {
    ...
    if (targetChannels.length === 0) {
      this.logger.warn(
        `No active channels for user ${userId} and notification type ${type}. Skipping notification.`,
      );
    ...
    this.processNotificationChannels(savedNotification, user)
      ...
      .catch((error) =>
        this.logger.error(
          `Error processing notification ${savedNotification.id}: ${error.message}`,
        ),
      );
      ...
      try {
        switch (channel) {
          ...
          case NotificationChannel.SMS:
            if (user.phoneNumber) {
              channelSuccess = await this.smsService.sendSms(
                user.phoneNumber,
                notification.message,
              );
            } else {
        ...
        this.logger.error(
          `Failed to send notification ${notification.id} via ${channel}: ${error.message}`,
        );
      }
    }
