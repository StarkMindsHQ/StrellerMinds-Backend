...
export class NotificationGateway {
  ...
  async handleNewNotificationEvent(payload: { userId: string; notification: Notification }) {
    this.logger.log(
      `Emitting new notification to user ${payload.userId}: ${payload.notification.id}`,
    );
  ...
  async handleNotificationReadEvent(payload: { userId: string; notificationId: string }) {
    this.logger.log(
      `Emitting notification read event for user ${payload.userId}: ${payload.notificationId}`,
    );
    this.server
      .to(payload.userId)
      .emit('notificationRead', { notificationId: payload.notificationId });
  }

  @OnEvent('notification.clicked')
  async handleNotificationClickedEvent(payload: { userId: string; notificationId: string }) {
    this.logger.log(
      `Emitting notification clicked event for user ${payload.userId}: ${payload.notificationId}`,
    );
    this.server
      .to(payload.userId)
      .emit('notificationClicked', { notificationId: payload.notificationId });
  }
