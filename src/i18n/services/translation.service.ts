...
export class TranslationService {
  ...
  async getTranslations(filters: TranslationFilters = {}): Promise<Translation[]> {
    ...
    return query
      .orderBy('translation.namespace', 'ASC')
      .addOrderBy('translation.key', 'ASC')
      .getMany();
  }
  ...
  async getTranslationStats(locale?: string): Promise<{
    ...
    for (const translation of translations) {
      ...
      stats.byNamespace[translation.namespace] =
        (stats.byNamespace[translation.namespace] || 0) + 1;
    }

    // Calculate completion percentage
    const publishedCount = stats.byStatus[TranslationStatus.PUBLISHED];
    stats.completionPercentage =
      stats.total > 0 ? Math.round((publishedCount / stats.total) * 100) : 0;
