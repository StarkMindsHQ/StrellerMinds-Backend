...
export class AccessibilityMetadataService {
    ...
    try {
      ...
      if (filters.hasViolations !== undefined) {
        if (filters.hasViolations) {
          ...
          query.andWhere(
            '(metadata.validationErrors IS NULL OR jsonb_array_length(metadata.validationErrors) = 0)',
          );
        }
      }
  ...
  async getComplianceStats(): Promise<{
    ...
    try {
      const [total, byLevel, byStatus, byContentType, violationsData, warningsData] =
        await Promise.all([
  ...
  private generateRecommendations(metadata: AccessibilityMetadata[]): string[] {
    ...
    const formsWithoutLabels = metadata.filter(
      (m) => m.contentType === ContentType.FORM && !m.formLabels,
    ).length;
    ...
    const lowContrastElements = metadata.filter(
      (m) => m.colorContrast && m.colorContrast.ratio < 4.5,
    ).length;
    ...
    const interactiveWithoutKeyboard = metadata.filter(
      (m) => m.isInteractive && !m.focusManagement?.focusable,
    ).length;

    if (interactiveWithoutKeyboard > 0) {
      recommendations.push(
        `Make ${interactiveWithoutKeyboard} interactive elements keyboard accessible`,
      );
    }
