...
expect.extend({
    ...
    toHaveValidationError(received, field) {
        const hasValidationError = received &&
            ...
            received.response.message.some((msg) => msg.toLowerCase().includes(field.toLowerCase()));
    ...
    toContainObject(received, object) {
        const pass = received.some(item => Object.keys(object).every(key => item[key] === object[key]));
        if (pass) {
            return {
    ...
    toHaveBeenCalledWithObjectContaining(received, object) {
        const calls = received.mock.calls;
        const pass = calls.some(call => call.some(arg => typeof arg === 'object' &&
